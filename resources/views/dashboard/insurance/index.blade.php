...
@section('content')
    ...
    <!--begin::Modal -->
    @include('dashboard.insurance.create')
    <!--end::Modal -->
        ...
        jQuery(document).ready(function() {
            ...
            table.DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'rate',
                        title: "{{ __('datatable.rate') }}"
                    },
                    {
                        data: 'discount_percentage',
                        title: "{{ __('datatable.discount_percentage') }}"
                    },
                    {
                        data: 'code',
            ...
            $(document).on('click', '#edit', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#kt_blockui_4_4').data('id', insuranceId);
                        $('#kt_blockui_4_4_modal').modal('show');
                    }
                });
            });
            $(document).on('click', '#kt_blockui_4_4', function(e) {
                e.preventDefault();
                var ajaxRoute = '{!! route('insurances.store') !!}';
                var insuranceid = $(this).data('id');
                var ajaxData = $('.kt_form').serialize();
                var ajaxMethod = 'POST';
                if (insuranceid) {
                    ajaxRoute = '{!! route('insurances.update', ['insurance' => '__id__']) !!}';
            ...
            $(document).on('click', '#updatestatus', function() {
                var insuranceid = $(this).data('id');
                $.ajax({
                    url: '/dashboard/admin/insurances/' + insuranceid + '/get-status',
                    type: 'GET',
                    success: function(response) {
                        ...
                        $('#saveStatusChanges').data('id', insuranceid);
            ...
            $('#saveStatusChanges').on('click', function() {
                var insuranceid = $(this).data('id'); // Get the data-id attribute
                var status = $('#select_status').val();
                $.ajax({
                    url: '/dashboard/admin/insurances/' + insuranceid + '/update-status',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,

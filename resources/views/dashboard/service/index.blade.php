...
@section('content')
    ...
    <!--begin::Modal -->
    @include('dashboard.service.create')
    <!--end::Modal -->
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $(document).on('click', '#kt_blockui_4_4', function(e) {
                e.preventDefault();
                var ajaxRoute = '{!! route('services.store') !!}';
                var serviceid = $(this).data('id');
                var ajaxData = $('.kt_form').serialize();
                var ajaxMethod = 'POST';
                if (serviceid) {
                    ajaxRoute = '{!! route('services.update', ['service' => '__id__']) !!}';
            ...
            $(document).on('click', '#updatestatus', function() {
                var serviceId = $(this).data('id');
                $.ajax({
                    url: '/dashboard/admin/services/' + serviceId + '/get-status',
                    type: 'GET',
                    success: function(response) {
                        ...
                        $('#saveStatusChanges').data('id', serviceId);
            ...
            $('#saveStatusChanges').on('click', function() {
                var serviceId = $(this).data('id'); // Get the data-id attribute
                var status = $('#select_status').val();
                $.ajax({
                    url: '/dashboard/admin/services/' + serviceId + '/update-status',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status,
